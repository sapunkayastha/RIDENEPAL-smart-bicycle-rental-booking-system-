...
// Splits a paid booking's total between the platform and the bike's
// vendor, using the current commission rate. Platform-owned bikes
...
async function applyCommission(bookingId: string) {
  ...
  await pool.execute(
    ...
    { commission, payout, id: bookingId },
  );
}
  ...
  .handler(async ({ data, context }) => {
    ...
    if (isComplete) {
      ...
      if (payment.extension_id) {
        ...
        await applyCommission(payment.booking_id);
      }
    }
